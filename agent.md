...
- The main menu can start the single-player game, open a private-server build flow that creates a room code after Done, or join a private room code when the game is served by the local WebSocket server. The world multiplayer panel's Create button also creates a private room code. Public multiplayer remains a placeholder.
...
- GitHub Pages/static hosting now disables private-room creation and shows a server-offline hint instead of presenting the static page URL as a friend link.
